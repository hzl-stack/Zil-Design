...
let utilFormat = require('util').format;
...
function TinyArgs(ctor, other, cleanErrors__) {
  ...
  this.options = opts;
  this.ctor = ctor;
  if (cleanErrors__) cleanErrors_();
  if (other) this.set(other);
}
...
TinyArgs.prototype = {
  set: function (other) {
    ...
    Object.keys(other).forEach(function (key) {
      if (opts.hasOwnProperty(key)) {
        let value = other[key];
        opts[key] = verifyType(key + ': ' + value, typeof opts[key], value);
      } else {
  ...
  parse: function (args) {
    ...
    if (args[0] === '') return this.parse_(args.slice(1));

    let s = args[0].slice(1);
    error("unexpected option '" + s + "',", "did you mean: '--" + s + "'?");
  },
  ...
  parse_: function (args) {
    ...
    for (i = 0; i < args.length; i++) {
      ...
      if (s.length > 1 && s[0] !== '-') {
        ...
        s = s.split('');
        if (a.length > 0) s[s.length - 1] += '=' + a.join(' ');
        this.parse_(s);
      } else {
        k = this.key_(s);
        if (k)
          this.options[k.key] = convValue(
            '-' + args[i],
            k.value,
            a.length > 1 ? a : a[0],
          );
  ...
  usage: function () {
    let punct = '  -, --  '.length,
      width = this.ctor.reduce(function (r, d) {
        let n = d[LONG].length + d[SHORT].length;
        return n > r ? n : r;
      }, 0);
    ...
    return this.ctor
      .map(function (d) {
        ...
        n = '\n' + indent(width + punct);
        tail = d.length < TEXT + 2 ? '' : n + d.slice(TEXT + 1).join(n);
  ...
  key_: function (arg) {
    if (this.keys) return this.keys[arg];

    let keys = {};
    this.ctor.forEach(function (a) {
      keys['-' + a[LONG]] = keys[a[SHORT]] = {
...
var convValueFunc = {
  ...
  string: function convValueString(option, def, value) {
    if (value === undefined) error("missing '" + option + "' value");
    return value;
  },
};
